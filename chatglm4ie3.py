'''
    基于chatGLM3完成IE任务（实体关系信息抽取） chatglm3-6B
...
def train():
    ...
    logger.info(
        "基于QLora 4-bit和NEFTUNE微调chatglm3实现IE任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps))

    logger.info("开始创建分词器...")

    pretrained_checkpoint = "uer/ZhipuAI/chatglm3-6b"
...
def retrain(checkpoint):
    logger = create_logger(name="retrain_log",
                           filename=checkpoint + "/chatglm4ie3.log")
    logger.info(
        "------------------------------------------------------------------------------------------------------------------------------------------")

    logger.info("RETRAIN LOGGING......")
    logger.info(
        "基于QLora 4-bit和NEFTUNE继续微调chatglm3实现IE任务，超参数有--num_samples %d --max_length %d --num_epochs %d --lr %e --batch_size %d --accum_steps %d" % (
            pargs.num_samples, pargs.max_length, pargs.num_epochs, pargs.lr, pargs.batch_size, pargs.accum_steps))
...
if __name__ == "__main__":
    ...
    if pargs.mode == "train":
        ...
        retrain("models/chatglm4ie3")
    elif pargs.mode == "infer":
        generator("models/chatglm4ie3")
