...
class MyDataset(Dataset):
    ...
    def __init__(self, tokenizer):
        ...
        prompt_prefix = "你是一个命名实体识别模型，请找出类别为\"治疗\"、\"身体部位\"、\"症状和体征\"、\"检查和检验\"和 \"疾病和诊断\"的实体，实体与类别之间用\"_\"连接，实体之间用\"；\"分割。文本："
        prompt_suffix = "\t实体有："
...
def train():
    mycheckpoint = "models/llama4ner1"
...
def generator(checkpoint):
    ...
    while intent:
        ...
        # 方法一：使用generate函数
        # askprompt = text_generation.get_chat_prompt('', ask, [], pargs.max_length, tokenizer)[0]
        ...
        entities = decoded_pres.split("[/INST]")[1].strip().rstrip("。").split("；")
        print("\n实体有：")
        ...
        entities = output['response'].strip().rstrip("。").split("；")
